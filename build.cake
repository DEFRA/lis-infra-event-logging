...
Task("Test")
    ...
    .Does(() => {
        ...
        {
            ArgumentCustomization = args => args
                .Append("-reportTypes:HtmlInline_AzurePipelines_Dark;Cobertura")
                .AppendQuoted("-assemblyfilters:+Defra.Lis.EventLogging.*;-*.Tests")
        };
        
        ReportGenerator(glob, outputDirectory, reportSettings);

        var coberturaReport = outputDirectory.Path.CombineWithFilePath("Cobertura.xml");
        var coverageDocument = System.Xml.Linq.XDocument.Load(coberturaReport.FullPath);
        var lineRate = double.Parse(
            coverageDocument.Root.Attribute("line-rate").Value,
            System.Globalization.CultureInfo.InvariantCulture);

        Information($"Production line coverage: {lineRate:P2}");
        if (lineRate < 0.90)
        {
            throw new CakeException($"Line coverage {lineRate:P2} is below the required 90.00% threshold.");
        }
    });
